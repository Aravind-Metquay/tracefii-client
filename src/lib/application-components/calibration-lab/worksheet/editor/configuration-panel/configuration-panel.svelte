<script lang="ts">
	...
	function getTitleInfo() {
			...
			return {
				title:
					currentActiveComponent.componentType === 'Table'
						? currentActiveComponent.tableComponent?.tableName
		...
		if (worksheetManager.getCurrentActiveFunction()) {
			return {
				title:
					worksheetManager.getCurrentActiveFunction()?.functionName || 'Function Configuration',
	...
	<div class="flex-1 overflow-y-auto">
		...
		{:else if currentActiveComponent}
			...
			{:else if currentActiveComponent.componentType === 'Table'}
				<TableConfiguration
					component={currentActiveComponent}
					onExpressionModal={handleExpressionModal}
				/>
