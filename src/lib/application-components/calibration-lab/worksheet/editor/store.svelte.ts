...
type ExcludeColumnPropsThatCannotBeEdited =
	| 'columnType'
	| 'columnId'
	| 'inputComponent'
	| 'selectComponent';
...
export function initializeWorksheet(worksheetData?: WorksheetType): WorksheetManager {
	...
	return {
		...
		updateBaseComponentProperties(
			...
			if (!component)
				throw new Error(
					'Component not found while updating properties' + ' ' + JSON.stringify(component)
				);
		...
		removeComponent(componentId: string) {
			...
			if (idx < 0)
				throw new Error(
					'Component not found while trying to delete' + ' ' + JSON.stringify(componentId)
				); // Fixed: changed <= to <
		...
		updateInputComponent(componentId: string, props: Partial<InputComponent>) {
			const component = this.getComponentById(componentId);
			if (!component)
				throw new Error('Input component not found while updating' + ' ' + componentId);
			if (component.componentType !== 'Input')
				throw new Error('Component is not an input component' + ' ' + JSON.stringify(component));
		...
		updateSelectComponent(componentId: string, props: Partial<SelectComponent>) {
			...
			if (component.componentType !== 'Select')
				throw new Error(
					'Found different component while updating select component' +
						' ' +
		...
		updateBaseColumnProperties(
			...
			if (!column)
				throw new Error(
					'Column or table not found while updating base col properties' +
						' ' +
						'Table & Column' +
						tableId +
						' ' +
		...
		updateInputColumnProperties(tableId: string, columnId: string, props: Partial<InputComponent>) {
			...
			if (!column)
				throw new Error(
					'Column or table not found while updating input col properties' +
						' ' +
						'Table & Column' +
						tableId +
						' ' +
		...
		updateSelectColumnProperties(
			...
			if (!column)
				throw new Error(
					'Column or table not found while updating select col properties' +
						' ' +
						'Table & Column' +
						tableId +
						' ' +
