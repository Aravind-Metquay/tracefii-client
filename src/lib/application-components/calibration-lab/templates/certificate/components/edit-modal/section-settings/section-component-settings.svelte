<script lang="ts">
	...
	function updateSectionName(newName: string) {
		if (selectedSection) {
			// Create a new sections array with the updated name
			const updatedSections = certificate.sections.map((section) =>
				section.id === selectedSection.id ? { ...section, name: newName } : section
			);

			// Update the sections (this will trigger the setter)
			certificate.sections = updatedSections;
							...
							onchange={(e) => {
								const updatedSections = certificate.sections.map((section) =>
									section.id === selectedSection.id
										? { ...section, visible: (e.target as HTMLInputElement).checked }
										: section
								);
								certificate.sections = updatedSections;
							}}
				...
				<button
					...
					onclick={() => {
						if (confirm(`Are you sure you want to delete "${selectedSection.name}"?`)) {
							const updatedSections = certificate.sections.filter(
