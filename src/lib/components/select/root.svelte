<script lang="ts">
	import { clickOutside } from '@/components/utils/event.js';
	...
	let { value = $bindable(''), size = 'medium', class: _class = '', children }: Props = $props();
...
{#if rootState.getIsActive()}
	<div
		in:fade|local
		out:fade|local
		class="fixed top-0 left-0 w-full h-full bg-kui-black bg-opacity-[0.4] lg:hidden z-[2000]"
	></div>
{/if}

<div use:clickOutside={() => rootState.setIsActive(false)} class="relative inline-block {_class}">
	{@render children()}
</div>
