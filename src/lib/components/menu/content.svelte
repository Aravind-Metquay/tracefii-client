...
{#snippet desktopSnip()}
	{#if rootState.getIsActive()}
		<div
			...
			class="absolute {rootState.getContentPosition()} {alightmentClass} z-[1000] {_class}"
			{...rest}
		>
			<div
				class="hide-scrollbar bg-kui-light-bg dark:bg-kui-dark-bg p-2 rounded-[12px] border border-kui-light-gray-200 dark:border-kui-dark-gray-400 shadow-sm scroll-smooth overflow-y-auto {_class}"
			>
				{@render children()}
			</div>
		</div>
